...
{
    public class DeleteTradeCommandHandler(ITradeQueryService tradeQueryService, ITradeRepository tradeRepository, IUnitOfWork unitOfWork) : IRequestHandler<DeleteTradeCommand, Result>
    {
        public async Task<Result> Handle(DeleteTradeCommand command, CancellationToken cancellationToken)
        {
            var trade = await tradeQueryService.GetTradeAsync(command.Id, cancellationToken);
