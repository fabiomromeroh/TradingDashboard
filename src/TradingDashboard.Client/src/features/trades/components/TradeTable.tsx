...
const columns: ColumnDef<Trade, unknown>[] = [
  { accessorKey: 'symbol', header: 'Symbol' },
  {
    ...
    cell: ({ getValue }) => {
      ...
      return (
        <span className={val === 'Long' ? 'text-green-600' : 'text-red-600'}>{val}</span>
      );
    },
  },
  {
  ...
  },
  {
    accessorKey: 'status',
    header: 'Status',
    cell: ({ getValue }) => <TradeStatusBadge status={getValue() as Trade['status']} />,
  },
  {
    id: 'actions',
    enableHiding: false,
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem className="text-destructive">Delete trade</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      );
    },
...
export function TradeTable() {
  ...
  if (isLoading) {
    return <p className="py-8 text-center text-muted-foreground">Loading trades…</p>;
  }
