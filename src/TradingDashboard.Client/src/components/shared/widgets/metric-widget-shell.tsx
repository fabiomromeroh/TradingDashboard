      ...
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            {badge ? (
              <Badge variant="secondary" className={cn("ml-1 rounded-full px-2 py-0 text-[10px]", toneClass(badge.tone))}>
                {badge.label}
              </Badge>
            ) : null}
          </div>

          <div className={cn("text-3xl font-semibold tracking-tight tabular-nums", valueClassName)}>
            {value}
          </div>
      ...
      {footerStats?.length ? (
        <CardContent className="flex flex-wrap items-center gap-x-4 gap-y-1 pt-0">
          {footerStats.map((stat) => (
            <div key={`${stat.label}-${stat.value}`} className="flex items-center gap-1.5 text-xs tabular-nums">
              <span className="text-muted-foreground">{stat.label}</span>
              <span className={cn("font-medium", toneClass(stat.tone))}>{stat.value}</span>
            </div>
          ))}
        </CardContent>
      ) : null}
